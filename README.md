...
## 🧱 Где взять модель (веса)

Чтобы проект работал, нужен файл весов `best.pt`.

1) Откройте страницу релизов проекта:

`https://github.com/<your-username>/SeatbeltDetection/releases`

2) Скачайте файл `best.pt` из assets релиза.
3) Положите его в проект по пути:

```
models/best.pt
```

После этого можно запускать Gradio и детект-скрипты.
